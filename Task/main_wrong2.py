...
then we're gonna have 36587 training sets and 15680 cross-validation sets
'''
Xval = X[:15679, 0]
Yval = X[:15679, 1]
X = X[15679:, :]
...
# def multivariateGaussian(X_input, mu, sigma, sigma2):
#     epsilon_rough = 1
#     # coefficient = 1 / (np.power(math.sqrt(2*math.pi), n_ts) * sigma)
#     size_X, useless_variable = X_input.shape
#     for i in range(size_X):
#         X_input[i,:] = [a - mu[i] for a in X_input[i,:]]
#         coefficient = 1 / (math.sqrt(2 * math.pi) * sigma[i])
#         secondPart = np.exp(- np.power(X_input[i], 2) / (2 * sigma2[i]))
...
# p = multivariateGaussian(X, mu, sigma, sigma2)
# print("epsilon: ", p)
...
# pval = Gaussian(Xval, mu, sigma, sigma2)
