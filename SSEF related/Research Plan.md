...
Solar energy has been put into the wide application for being secure, clean and available on Earth throughout the year.[^4] Many countries have considered using such renewable energy sources into the electricity grid and problems then come out as the supply of the energy source is not steady enough.[^3] Precise solar radiation prediction is key to successful integration of solar power as it allows optimal construction of additional photovoltaic (PV) system.

Currently, Weather Research and Forecast (WRF) model[^14] has been widely used for the weather forecast in the study [^18]  [^7]. Additionally, the model has been proved to be successful in power output forecasting in the context of Smart Grids and Renewable Energy on [^19]. However, the WRF model has its limitation of long running time [^20][^21] and it took three weeks in total to generate the whole dataset of solar radiation data at 62 different locations across one year used in this study.
...
The original data is the solar radiation data at 62 different locations across the country generated by the WRF model for the period 1 Jan 2018, 8 am - 1 Jan 2019, 8 am. It consists of four columns and processed to the format that python can accept.

In order to have a general idea about solar radiation potential in Singapore, graphs of Direct Solar Radiation (SWDIR) and Diffuse Solar Radiation (SWDIF) values at various locations between different time intervals (a day, a week and a month namely) will be plotted. As the predictions are in terms of CSR, defined as $CSR = SWDIF / (SWDIR + SWDIF)$ with its range from 0 to 1 by definition, the NaN values (not a number) of CSR with both SWDIR and SWDIF zero should be removed or converted to a usable format. The whole dataset is going to be split into training sets and test sets in the ratio of 7:3.
